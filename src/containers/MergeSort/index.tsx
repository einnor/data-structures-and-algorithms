...
import { MergeSort } from '../../lib/Sorting/Algorithms';
...
const MergeSortImplementation = () => {
  ...
  const sort = (): void => {
    ...
    mergeSort.set(input);
    setState({ ...state, results: mergeSort.sort(), swaps: mergeSort.swaps, comparisons: mergeSort.comparisons });
  }
  ...
  const switchContent = (): ReactNode => {
    ...
    switch (selectedTab) {
      ...
      default:
        return (
          <>
            <h2 style={{ marginBottom: 20 }}>Merge Sort ALgorithm</h2>
  ...
  const generateOrderedList = () => {
    setState({ ...state, value: mergeSort.generateOrderedList(parseInt(state.size, 10)).join(',') });
  }

  const generateRandomList = () => {
    setState({ ...state, value: mergeSort.generateRandomList(parseInt(state.size, 10)).join(',') });
  }
...
export default MergeSortImplementation;

...
import { QuickSort } from '../../lib/Sorting/Algorithms';
...
const QuickSortImplementation = () => {
  ...
  const sort = (): void => {
    ...
    quickSort.set(input);
    setState({ ...state, results: quickSort.sort(), swaps: quickSort.swaps, comparisons: quickSort.comparisons });
  }
  ...
  const switchContent = (): ReactNode => {
    ...
    switch (selectedTab) {
      ...
      default:
        return (
          <>
            <h2 style={{ marginBottom: 20 }}>Quick Sort ALgorithm</h2>
  ...
  const generateOrderedList = () => {
    setState({ ...state, value: quickSort.generateOrderedList(parseInt(state.size, 10)).join(',') });
  }

  const generateRandomList = () => {
    setState({ ...state, value: quickSort.generateRandomList(parseInt(state.size, 10)).join(',') });
  }

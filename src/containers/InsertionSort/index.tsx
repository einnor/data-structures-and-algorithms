...
const InsertionSortImplementation = () => {
  ...
  const sort = (): void => {
    ...
    insertionSort.set(input);
    setState({ ...state, results: insertionSort.sort(), swaps: insertionSort.swaps, comparisons: insertionSort.comparisons });
  }
  ...
  const switchContent = (): ReactNode => {
    ...
    switch (selectedTab) {
      ...
      default:
        return (
          <>
            <h2 style={{ marginBottom: 20 }}>Insertion Sort ALgorithm</h2>
  ...
  const generateOrderedList = () => {
    setState({ ...state, value: insertionSort.generateOrderedList(parseInt(state.size, 10)).join(',') });
  }

  const generateRandomList = () => {
    setState({ ...state, value: insertionSort.generateRandomList(parseInt(state.size, 10)).join(',') });
  }
...
export default InsertionSortImplementation;
